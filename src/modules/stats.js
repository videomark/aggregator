...
class Stats {
  ...
  async stats() {
    ...
    const statsCollection = await this.wrapper.collection(
      this.options.statsCollection
    );

    const allCollectionName = `${prefix}${config.get(
      "db.aggregation_collection_suffix.hour"
    )}`;
    const serviceCollectionName = `${prefix}${config.get(
      "db.aggregation_collection_suffix.service"
    )}_${config.get("db.aggregation_collection_suffix.hour")}`;
    const countryCollectionName = `${prefix}${config.get(
      "db.aggregation_collection_suffix.country"
    )}_${config.get("db.aggregation_collection_suffix.hour")}`;
    const subdivisionCollectionName = `${prefix}${config.get(
      "db.aggregation_collection_suffix.subdivision"
    )}_${config.get("db.aggregation_collection_suffix.hour")}`;
    const ispCollectionName = `${prefix}${config.get(
    ...
    const procs = [
      ...
      this.statsSubdivisionDays([
        subdivisionCollectionName,
        statsCollection,
        cb,
      ]),
      this.statsSubdivisionHours([
        subdivisionCollectionName,
        statsCollection,
        cb,
      ]),
